...
export interface LineVM {
   ...
   *  structured surface should prefer `body`/`source`/`age` instead (Epic 045
   *  S1): welding source/age into displayed copy is what made the "just now"
   *  ×11 defect impossible to relocate or collapse (A3). */
  text: string
  /**
   * The hedge + value alone — no source, no age (Epic 045 S1 AC-1.1). Optional:
   ...
   * datetime (Epic 045 S1 AC-1.1/1.4). Optional for the same reason as `body`.
   */
  age?: string
