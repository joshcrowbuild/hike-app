...
*Cross-cutting thread T6 (Workplan, thread T6 — line 25). Design v0.1 — June 24, 2026. Builds on `stage-1-data-sources.md` §5 (per-source license obligations), Decision Log §27 (ODbL handling, off-limits sources) / §11–§12 (commons) / §14 (retention/deletion) / §28 (Stage-2 separability) / §30–§31 (the Stage-5/6 commons fork — **BUILT by Epic 010**; see §4.1). The gate on Stage 9's public release.*

> **STATUS UPDATE (2026-06-26):** The de-identified **commons fork write is BUILT** (Epic 010; gap-audit C1 CLOSED) and the **CorpusSource / LiveAdapter source seams are BUILT** (Epics 012/013; C5/C6 CLOSED). Read every "designed, not yet built / not yet built / pending" reference to the fork write and the source seams **below** as *historical* — they ship and accrete today. What remains genuinely future (and what this thread correctly gates) is the **Stage-9 public-commons release**: the aggregation/read half, the ODbL publication posture, and the consent gate. The design narrative below is retained as provenance.
>
