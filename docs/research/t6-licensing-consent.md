...
| **NR** (no-redistribute) | Personal/reference use only; **never** in a distributed product | VA DCR Conservation Lands, Fairfax County (verify), PATC | **Never** crosses the publication boundary — enforced as a hard gate (§6, G-4). |
...
|---|---|---|---|
| **A — Commons under ODbL** | Publish the aggregated database under ODbL: attribution, share-alike, machine-readable, document alterations | We give up proprietary control of the *aggregate statistics* (not user data — that's de-identified bands) | **🔶 Recommended default.** The commons is a calm public-good utility (CLAUDE.md framing), not a moat (Stage-1 §2: "AllTrails has no trail-data moat; we don't either"). The aggregate *being open* is consistent with the project's stance, costs us nothing we were monetizing, and is the simplest to defend. |
...
**Recommendation: default to A (ODbL the commons), with C as the always-safe fallback for any artifact where ODbL is awkward.** Why: the project explicitly *isn't* building a data moat (Stage-1 §2), treats the commons as a public good (§12), and an ODbL commons is the lowest-discipline, highest-defensibility path. B's separability discipline is real engineering debt for a benefit (proprietary aggregate) we don't want. **This is a 🔶 to confirm with counsel at the G-7 gate**, because the precise boundary between "ODbL-clean aggregate" and "still a Derivative Database needing the full ODbL machine-readable offer" is the one place a lay reading isn't safe.
...
| **VA DCR Conservation Lands** | Signed agreement — **no redistribution / no for-profit** | **NR** | **Never in a distributed product** | No — personal/reference only; hard-gated (§6 G-4) |
...
- **The architecture already has the seam.** Decision Log §29 (live adapters, each `(loc)→verified fact|None`) and the device seam (`device-integration-seam.md`) establish the pattern: **every external source sits behind a normalized adapter contract, selected by config.** T6 extends that pattern with a *license obligation* on each adapter.

### 3.3 The discipline made concrete — six rules

| # | Rule | Why | Status |
|---|---|---|---|
| **SW-1** | **Every source is an adapter behind a normalized contract** (mirrors §29 live adapters + the device seam). A source's license/auth/transport is the adapter's private business; downstream sees normalized facts + a class tag. | Swapping a source = swapping one adapter, zero downstream change. This is *already* the architecture — T6 just forbids exceptions. | ✅ (confirms §29) |
...
The commons fork is already designed (Decision Log §30 / §31): on episode creation, a de-identified twin is written to a `:CommonsObservation` with the person link severed, endpoints trimmed (250m), and pace bucketed into capability bands. **Crucially, the fork happens at write time, but the data is dormant until volume (§12: "accretes from day one, dormant until volume").** This gives consent two distinct moments:
...
```
   ...
   │  COMMONS FORK    │   BUT — gated by CONSENT-TO-CONTRIBUTE (§4.2).
   │  (write-time)    │   No consent → no fork. The private episode is unaffected.
   └────────┬─────────┘
            │  (dormant, below k)
            ▼
...
| **Revocable (forward)** | Toggling off **stops future forks immediately** and removes any *still-dormant* (below-k, not-yet-aggregated) observations. | Revocation must be *real* for everything not yet irreversibly mixed (§14: "revocation is only real if nothing was copied"). |
...
Even with full commons consent, these remain structurally true (they're not consent-gated, they're invariant):

- The **person→observation link is severed at write** regardless (§28 decision 6: `:CommonsObservation` *guaranteed never edge-linked to a `:Person`*). Consent authorizes the fork; it does **not** authorize keeping the link.
- **Endpoints are trimmed** regardless (the re-ID vector — §12).
- **Raw pace is bucketed to a band contributor-side** before the commons write (§31) — the commons never sees raw biometrics, consent or not.
- **Nothing surfaces below k** (§7) — consent doesn't lower the floor.

Consent is the gate on *whether the de-identified twin is written*; the de-identification itself is non-negotiable mechanism (Rule #8). The two are independent layers of protection.
...
| **5–6** (memory/watch) | The **commons fork is consent-gated** (SW-2 outbound); dependent data excluded (R-1); fork severs link + trims + bands (already ✅ §30/§31) | 🔶 (consent gate on the fork — §4) |
...
The thread's discipline: **each upstream stage must not foreclose a downstream T6 option.** Stage 2 kept OSM separable so Stage 9 *can* choose Collective-DB. Stages 5–6 fork de-identified so Stage 9 *can* aggregate. T6's job is to ensure that chain never breaks.
...
| **G-9** | **The de-identification invariants hold regardless of consent**: link severed at write, endpoints trimmed (250m), pace banded contributor-side, **nothing below k**. | The Stage-9 privacy tests (sibling of §14 / §17 security tests) pass: the commons write *never* retains a person link; endpoint-trim *always* fires; no sub-k stat surfaces. | Rule #8, §28 decision 6, §12 | ❓ |
...
| **G-11** | **Forward revocation works**: toggling consent off stops future forks and removes still-dormant (below-k) observations. | A test: revoke → new episodes don't fork → below-k twins are gone; above-k aggregates correctly remain (and that limit was disclosed per G-8). | §4.2, §14 | ❓ |
...
| **T6-3** | **The commons-public form (ODbL the aggregate / Collective-DB / Produced-Work only) defaults to ODbL the aggregate**, with Produced-Work as the always-safe fallback — **confirmed with counsel at gate G-6.** Why: no data moat (Stage-1 §2), public-good framing (§12), lowest-discipline path. | 🔶 (counsel at G-6) |
...
| **T6-5** | **Swappability discipline (SW-1…SW-6):** every source is a config-selected adapter declaring its `license_class`; no source is sole-support except authoritative-by-nature ones (which degrade per #1); **NR sources never enter the corpus adapter registry**; a source is license-cleared before it gets an adapter. | ✅/🔶 (extends §29) |
| **T6-6** | **Commons-contribution consent is opt-in, default-off, separate from person-grants, informed, separately revocable, audited.** The consent gate sits on the **fork (write-time)**; no consent → no `:CommonsObservation`. | 🔶 |
...
- **The de-identification mechanism** (sever / trim / band) — Stage 9 (undesigned); already specified in §30/§31. T6 owns the *consent gate on it*, not the mechanism.
...
*Primary legal references: [ODbL 1.0](https://opendatacommons.org/licenses/odbl/1-0/) (§4.3 attribution, §4.4 share-alike, §4.5 Collective vs. Derivative Database) · [OSMF Licence & Legal FAQ](https://osmfoundation.org/wiki/Licence/Licence_and_Legal_FAQ) · [OSMF Attribution Guideline](https://wiki.osmfoundation.org/wiki/Licence/Attribution_Guidelines). Source license details: `docs/research/stage-1-data-sources.md` §5 + Appendix B. Schema separability: `docs/research/stage-2-schema.md` (Decision Log §28). Commons fork + consent disclosure: Decision Log §11–§12, §14, §30–§31. Off-limits sources verified 2026-06-19; re-check before any public release (ToS change often).*
