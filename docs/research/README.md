...
| [trail-connectivity-loops.md](trail-connectivity-loops.md) | `ACTIVE` (pre-epic exploration; backlog B010) | Trail connectivity & loop composition: three-layer problem definition (topology · composition · judgment), gap audit vs. today's flat-trail graph, open-source landscape (OSM relations, AllTrails' derived segment graph, GraphHopper round-trip, jogging-routes/AOP literature), the bad-loop screen (TSI tags + SAR incident record), the "worth recommending" rubric, and a one-region spike proposal gated on Phase-A identity. **Read when** scoping junction/`:Route` topology, loop composition, or the B005 routing substrate. |
