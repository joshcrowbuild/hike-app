...
> **What this stage does NOT do:** it does not switch the commons public. The forked write (T3) has accreted `:CommonsObservation` rows since Phase 0; **aggregation stays dormant until volume**, and **public exposure is gated by T6** (§8). Stage 9 is the design that turns the accreted substrate into trustworthy aggregate statistics — and the privacy proof that lets it ship.
...
The commons is a statistical layer derived from many traversals — but those traversals are matched to trails whose geometry is **OSM-derived** (the spine, Stage 1 §27). A public commons cross-referenced against OSM geometry is a **Derivative Database** → ODbL share-alike triggers on public conveyance. **Three resolved escape routes** (Decision Log §27, §28; pick before launch, do not relitigate):
...
> **◆ Phase-3 commons design complete** — the de-identification proof, the k=floor unification, contributor-side banding, the pace-calibration model (first) + the emergent-attribute template, the snapshot-differencing caveat, and the DP posture are fully specified. **Build is gated by volume (dormant aggregation) and by T6 (ODbL + consent) before anything goes public.** The forked write (T3) has been accreting since Phase 0; Stage 9 is what turns that substrate into trustworthy, unlinkable aggregate statistics when the volume and the consent are both there.
