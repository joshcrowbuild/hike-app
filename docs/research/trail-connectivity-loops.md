...
**Last verified:** 2026-07-09 · **Owner:** research · **Status:** `ACTIVE` (pre-epic exploration; backlog entry B010)

> **Status:** analysis only — nothing here is built or committed to. Grounded against the repo at `main` (b7d876d) and external sources fetched 2026-07-09; external claims carry URLs, repo claims carry `file:line`. Unverified items are flagged inline.
...
**First step — a bounded spike, not an epic** 🔶: offline notebook against one region's already-fetched geometry (no schema change, no pipeline change):
...
**Open questions** ❓
- Where do *connector* ways (fire roads, horse trails, the odd road shoulder) live in the corpus? Today's fetch takes `highway~path|footway|track|bridleway|steps` **with names** — unnamed connectors may be absent entirely; the spike must measure this first (it may be the actual long pole).
...
## 9. Source appendix (external, fetched 2026-07-09)
