...
**The one-line snapshot:** the build quality is real and invariant-faithful; the sophisticated personal-intelligence machinery runs on **mock episodes for one seeded user** because the intake is empty (no real auth, no HTTP episode creation, no history import, no search, one unverifiable region); the loudest differentiator (independence-checked corroboration) is currently a **constant 1**; and the operational substrate a live multi-user app needs is **absent while the app is deployed**.
...
| CDP | One-line | Phase |
|---|---|---|
| **01** | Independence-checked corroboration (walk to distinct origins) | A *(spike first)* |
...
**Goal.** Make every claim the engine **already renders** actually true before widening the door. Corroboration is a constant 1 (`engine.py:172`), the slug-merge can silently fuse two distinct same-source trails, three owned reads are guarded only by convention, and the confidence math averages instead of taking the weakest link. Fix the substrate the moat stands on.
...
**Includes.**
- **CDP-01 independence-checked corroboration** — Verifier probes *all* adapters per data-kind (not break-on-first-success), walks to distinct ORIGIN nodes, passes a real count to `for_fact` at `engine.py:172`, never re-ingests its own output. **Run the CDP-01 feasibility spike FIRST**; safe default if origin metadata is unrecoverable = treat-as-shared, count as 1.
...
- **BLOCKING substrate audit** of `_build_canonical_id` slug-collision (`ingestion/pipeline.py:72-83`). Two distinct collision modes, both able to fuse distinct trails — audit both: **(a)** short identical slugs — the sha1 suffix fires *only* when `len(slug) > 40`, so two same-source trails whose names slugify to an identical ≤40-char string collide with **no guard at all**; **(b)** long shared-prefix truncation — even past the guard, the id keeps only `slug[:33]` + a 6-char hash, so two same-source names sharing a long common prefix can still truncation-collide. Plus the **1643→1458 conflation delta** (~185 collapsed `canonical_id`s — *asserted-but-not-reproducible-from-`main`*, same provenance class as the Aura corpus claim; it can only be audited row-by-row once the re-runnable ingest verification lands — see *Dependencies* below). Extend the hash-suffix guard to short slugs *and* lengthen the kept-prefix so truncation can't fuse; make every merge additive/reversible/degree-guarded (**CDP-14**); flag-on-ambiguous-merge.
...
- **Doc-drift fixes** (cheap, load-bearing) — apply_schema "untracked", epic-index vs roadmap on 016/017, phantom epics 007/008. *(The runbooks router row + design-system §9.2 are already fixed in the working tree.)*

**Dependencies.** None — this is the foundation. The CDP-01 spike gates *how far* corroboration can go, not *whether* Phase A ships. **One internal ordering:** the slug-collision audit's *data* (the live Aura corpus) isn't reproducible from `main`, so the re-runnable ingest verification (formerly a Phase-E item) must ship *first within this phase* before the audit can run and the back-fill can be scoped.

**Exit criteria.** Corroboration is a real distinct-origin count end-to-end (or explicitly degraded to treat-as-shared with the spike documented); the ingest is re-runnable from `main` and the slug-collision audit (both collision modes) passes with **zero silent same-source merges** — with any required `canonical_id` back-fill applied so no `Episode`/`Belief`/grant edge is orphaned; an unscoped owned read fails CI; MIN-fusion is the confidence math in code; the doc drifts are corrected.
...
- **Epic 009** — golden-trip set + cassettes/VCR fixtures + N-run regression gate + LLM-judge, in CI against the *real* engine; plus a **Brier/reliability calibration hook** (a 0.8 true ~80% of the time), which needs CDP-01's real corroboration to be meaningful. **Degrade branch:** if the CDP-01 spike returns "origin metadata unrecoverable," the calibration hook **calibrates authority + freshness only and defers the corroboration term** — the harness has a defined shape under both spike outcomes, so a no-go spike result never strands Phase B's deliverable.
...
**Includes.**
- **Managed auth (R3 / B004)** replacing the shared dev-secret — choose a provider (Supabase/Clerk/Auth0), issue real per-identity sessions; `ScopedSession` stops trusting `viewer_id` verbatim; adopt the **CDP-12** shape (Zanzibar tuple + userset-rewrite + consistency-token) so a future revoke takes effect on next VIEW.
...
**What good looks like.** The top card reads "Marginal — go if you start before noon"; one tap shows the binding constraint (NWS thunderstorms, named + timestamped) and the creek ford "confirmed by 2 independent sources"; a ruled-out trip sits in a "Ruled out (1)" tray naming exactly why (fire closure, NPS source); structural distance is quietly marked un-reverified, not dressed as live. Readiness is a hedged capability band that tightens the effort floor but never reorders. The whole answer is readable in under a minute and the success state is **the user leaving**. *(CDP-01 degrade branch: if origin metadata is unrecoverable, the ford line reads "single authoritative source (USGS)" instead of "confirmed by 2 independent sources" — the verdict, the named binding constraint, the ruled-out tray, and the four silence states are all unchanged, so the moment survives a no-go spike without a rewrite.)*
...
- **CDP-01 feasibility spike** — can NWS/USGS/AirNow/FIRMS/RIDB expose recoverable distinct-origin metadata? This single answer gates whether the headline moat is real or degrades to a relabeling exercise. **Answer it FIRST.**
- **Implement real corroboration** — Verifier probes *all* adapters per kind (not break-on-first-success), walks to distinct ORIGIN nodes, passes a true count to `for_fact` at `engine.py:172` (today the hardcoded default 1); MIN/weakest-link fusion (CDP-06).
- **BLOCKING substrate audit** — `_build_canonical_id` slug-collision (`pipeline.py:72-83`, short-slug case unguarded) + the unaudited 1643→1458 delta; make merges additive/reversible/degree-guarded (CDP-14), flag ambiguous merges.
- **Fix the doc drifts NOW** — apply_schema "untracked", epic-index vs roadmap on Maps 016/017, phantom epics 007/008 (runbooks router row + design-system §9.2 already fixed in this pass); the whole vision must stand on fresh memory.
...
1. **CDP-01 corroboration feasibility spike** →
2. **Real independence-checked corroboration + MIN fusion in the engine** *[makes the headline claim TRUE]* →
3. **Slug-collision substrate audit** *[makes the data the claim rests on TRUE]* →
...
1. **Substrate before surface — fix what's false before building what's new.** The product already renders a corroboration claim that is a constant 1 (confirmed: `for_fact` defaults `corroboration=1`; `engine.py:172` never overrides) — the exact feed-counting failure 13 independent high-stakes domains name as cardinal — over a corpus that may have silently fused real trails (the unaudited 1643→1458 slug-merge). A beautiful verdict layer built on constant-1 corroboration and a possibly-corrupted substrate is **confidently-wrong safety at a larger blast radius**, so Phase A precedes Phase D. The honesty primitives *are* the product; making them true is non-negotiable Phase 1.
...
1. **CDP-01 origin-metadata feasibility — the moat-or-relabel fork.** If NWS/USGS/AirNow/FIRMS/RIDB do *not* expose recoverable distinct-origin metadata for most source pairs, does "independence-checked corroboration" degrade gracefully to treat-as-shared/count-as-1 with honest disclosure, or do we invest in a corroboration substrate (cross-referencing station IDs / issuing offices) to recover independence? This single answer reshapes the headline differentiator. **Decide FIRST.**
2. **Corroboration honesty when unimplementable.** If the moat degrades, do we KEEP the corroboration axis (honestly showing "counts as 1") or REMOVE it from the formula to stop implying a capability we don't exercise? The research warns an advertised-but-fake axis is itself the cardinal sin.
...
4. **Managed auth provider (Supabase vs Clerk vs Auth0 vs roll-your-own).** Must support the CDP-12 consistency-token / revoke-on-next-view semantics the grant system (Phase F) needs — so the choice is made for the multiplayer endgame, not just login. **Decide before Phase C.**
...
| **Update (de-rot)** — drop the stale "commit untracked `apply_schema.py`" follow-up (4 spots: ~lines 78/100/116 + the v9 header ~line 9); it **is** tracked | `docs/process/roadmap.md` | ⚠ needs-confirmation | PM |
| **Update (drift fix)** — flip `Status:` header to **DONE** so the generated epic index reconciles with the roadmap | `docs/epics/epic-016-maps-and-terrain.md` (DEFINED→DONE), `docs/epics/epic-017-terrain-elevation-enrichment.md` (IN_PROGRESS→DONE) | ⚠ needs-confirmation | PM |
...
**Lane hand-off (load-bearing, not optional).** The ⚠-marked rows — the epic-016/017 `Status:` flips, the roadmap apply_schema de-rot, the workplan freshness header, and the plan-analysis archive — are **PM/PO-lane tasks**, *filed* from here but **never executed from the vision lane**. The vision-PM proposes; the PM/PO disposes. The safe (docs/build-lane) rows — the `docs/runbooks/` router add, the parallel-integration-runbook archive move, the archive-index append — may proceed in this pass.

**Net change still to apply (vs the working tree, not `main`):** commit the two new docs + the README (vision/strategy + runbooks router) and design-system edits already in the working tree; archive 1 (plan-analysis) + repoint its 2 dangling backtick refs (`CLAUDE.md:18`, `novelty-filter-spec.md:3`); move 1 (parallel-integration-runbook); flip 2 epic `Status:` fields (PM-lane); de-rot the roadmap follow-up (PM-lane); refresh the workplan header (PM-lane); append 2 rows to the archive index. Fewer, clearer docs; **nothing abandoned**; the memory the vision stands on is made fresh.
