...
| CDP | One-line | Phase |
|---|---|---|
| **01** | Independence-checked corroboration (walk to distinct origins) | A *(spike ✅ DONE — now wire corpus `SAME_AS` origins into `for_fact`)* |
...
**Includes.**
- **CDP-01 independence-checked corroboration — spike ✅ DONE, now the scoped wiring task** ([`../research/cdp-01-corroboration-feasibility-spike.md`](../research/cdp-01-corroboration-feasibility-spike.md): moat real, locus = corpus `SAME_AS` layer). The work, now grounded:
  - **(a) Wire corpus corroboration into `for_fact`** — count distinct `SourceRecord.source` per `SAME_AS` cluster (per-attribute where survivorship differs) and pass it as the real `corroboration` arg at `engine.py:172`; this is the moat and it's a graph read we can already do.
  - **(b) Honestly label live single-source facts** — Verifier probes *all* adapters per kind (not break-on-first-success) so a future 2nd provider gets the check for free, but where a kind structurally has one government source, the fact reads "single authoritative source (counts as 1)," carrying its distinct-origin id (USGS gauge / NWS office+grid / FIRMS satellite). AirNow is an aggregator → honestly one shared origin.
  - **(c) Never re-ingest our own output** as external evidence. The only genuinely-hard case the spike found is AirNow; everything else has a recoverable origin id.
...
- **Wire real corroboration (the scoped task the spike unblocked)** — count distinct `SourceRecord.source` per `SAME_AS` cluster and pass it as the true `corroboration` arg to `for_fact` at `engine.py:172` (today the hardcoded default 1); Verifier probes *all* adapters per kind (not break-on-first-success) and labels structurally-single-source live facts honestly, capturing each one's origin id; MIN/weakest-link fusion (CDP-06).
...
2. **Wire `SAME_AS`-cluster corroboration + MIN fusion into the engine** *[makes the headline claim TRUE — the scoped task the spike unblocked]* →
