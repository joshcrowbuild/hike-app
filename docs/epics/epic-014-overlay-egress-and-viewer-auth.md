...
- [x] All ACs covered by at least one passing test (named per process doc) — `tests/test_overlay_egress.py`, `tests/test_viewer_auth.py`
- [x] AC-1.2, AC-1.3, AC-3.1 each demonstrably FAILED before the fix and PASS after (the regression guards)
- [x] `make check` green (ruff + mypy + pytest) before commit
- [x] Overlay-carrying taste-ranking (`curate`) resolution passes `touches_private_overlay=True` at every path; the `judge`/eval call (same `judgment` tier, no overlay) and the mechanical/intent resolution are left unchanged
- [x] `build_runtime` docstring documents the "`viewer_id` is already authenticated" contract (and the AC-2.4 privacy-routing note)
- [x] Edge guard fails closed: non-anonymous `viewer_id` rejected without the configured dev secret, on both `/plan` and `/episode/{id}/outcome`
- [x] `ADVENTURE_DEV_VIEWER_SECRET` wired through `Settings.from_env()`, absent by default, documented in `.env.example`
- [x] The gap-audit (`docs/research/architecture-gap-audit-2026-06.md`) and additions-proposed (`docs/research/decision-log-additions-proposed.md`) docs are committed alongside this epic (they are not yet on the build branch), so the §40 citation and the "mark remediated" step below resolve
- [x] Targeted self-review agent run (4-lens adversarial workflow over `orchestration/engine.py`, `orchestration/providers/registry.py`, `api/app.py`, `api/schemas.py`); 0 CRITICAL / 0 MODERATE; the 2 confirmed LOW findings both addressed (docstring ACs pinned by test; non-ASCII secret compared as bytes)
- [x] `docs/research/decision-log-additions-proposed.md` §40 C3 + C4 marked remediated, pointing at this epic (treating §40's `engine.py:191-192` line cite as illustrative, since this epic edits that call site); `docs/epics/README.md` row added
- [x] Atomic commits (privacy-routing fix · edge auth guard · tests · doc updates), each its own commit

---

**Implementation note — AC-1.4 trigger deliberately broadened (review LOW-2).** The
overlay-carrying judge is selected whenever `viewer_id != "anonymous"` **OR**
`personal_context` is non-empty — broader than AC-1.4's literal overlay-only trigger.
An authenticated session is private by default (Rule #5), so its ranking stays
on-device even when this run's assembled context happens to be empty: defense-in-depth
against an assembly path that silently empties `personal_context` for a viewer who does
have overlay. This is strictly **more** conservative than the overlay-only trigger
(never routes overlay to the cloud; only over-routes non-private free-text to local) and
is pinned by `test_s2_ac1_non_anonymous_empty_overlay_uses_local_judge`. The anonymous
no-overlay path (incl. anonymous intent.profile-only free-text) still resolves per config
(cloud yardstick), exactly as AC-1.4 requires.
