...
> **Build prerequisite (honest scope — read before sequencing).** Stage 7 depends on Stages 5–6 being built: a real personalized, watch-enriched flow to evaluate. Per `docs/epics/README.md`, Epic 001 is DONE but **002/003 are DEFINED-not-built and 004/007 are BACKLOG**. Therefore **only S1–S4, S6, S8 (replay-only), and the unit half of S7 are buildable now.** S5 judge-calibration (AC-5.6), the S7 sensitivity-routing eval-loop check (AC-7.3), and **all of S9** are blocked on Epics 002/003/004 landing. This epic is *defined* in full so the access/eval seams are built test-first, but it **cannot reach DONE before those epics land** — the DoD reflects this.

> **Seam-name note (memory hygiene, CLAUDE.md "wrong memory is worse than none").** The decision-log canonically names the access wrapper **`scopedQuery(viewer)`** (§28/§29/§30); the code ships it as **`GraphClient.scoped_session(viewer_id, granted_ids=())` → `ScopedSession`**. This epic uses the **code** name throughout (correct for a build epic). The log's `scopedQuery` is the stale label and should be reconciled to `scoped_session` in a future log-tidy pass.
...
**AC-2.7:** The viewer in every scenario flows through `GraphClient.scoped_session(viewer_id, granted_ids)` → `ScopedSession` (rule #4) — the eval uses the **same access seam as production, never a privileged bypass**. The `granted_ids` parameter exists today, so grant-bearing scenarios pass real grant sets through it now (only the grant-*stop* enforcement semantics in AC-7.5 are Stage-8-deferred).
...
**AC-3.8:** A `promote_episode()` path is **stubbed and documented** (real watch-episode promotion is Stage 6, §10.2): selected `Episode`s become golden trips **de-identified, scrubbed of biometrics per rule #5 before entering the shared eval set**. The golden set must stand **without** watch data (rule #6 — watch is enrichment).
...
**AC-4.4 (§4.1d disclosure completeness):** Every fact below the confidence floor (decision-log §7), or carrying adapter `disclosures` (nearest-gauge distance, FIRMS "thermal anomaly ≠ fire", AirNow "preliminary"), renders **with** its hedge/flag — the *presence* of the disclosure string is a code check (whether it *reads* honestly is the scoped judge, S5).
...
**AC-7.1 (no-ungranted-node, rule #4):** A property-based test (decision-log §28) fuzzes random grant sets × random viewers, asserting **every** query path goes through `GraphClient.scoped_session(viewer_id, granted_ids)` and a viewer with scope S **never** receives a node outside S. Uses the real `granted_ids` parameter (exists today). The eval-loop version asserts the rendered `Feed` contains **zero** nodes the scenario's viewer wasn't granted. **Pass rate must be 1.0.**
**AC-7.2 (commons person-link severance, rule #5):** Three code assertions (decision-log §28/§30): no `:CommonsObservation` is edge-reachable to a `:Person`; the forked write **severs the link in the same transaction** as the episode; **endpoint-trimming fires** (the 250m strip present, the raw track absent). Zero-tolerance. 🔶 The 250m *value* is tuned in Stage 9; this test asserts *that* it fires.
**AC-7.3 (sensitivity-routing — BLOCKED on Stage 6 watch build):** Any flow touching the private overlay routes via the provider seam's sensitivity resolution (decision-log §29 "route by data sensitivity" + §31), forcing a **local** provider — raw FIT/HR/GPS/biometric content **never** appears in a cloud-bound prompt. A test inspects the captured prompt payloads + the resolution. (This is the real seam behind §29's route-by-sensitivity decision; §31 applies it to watch data.)
**AC-7.4 (privacy-adversarial scenario):** Scenario 6 — a viewer with **no grants** (`granted_ids=()`), crafted to *try* to surface another member's episode or raw biometrics — runs the above invariants inside the eval loop across N runs, not only in unit tests.
...
**AC-9.4:** This story is **gated on Stage 5/6 build** — Epics 001 (belief store, DONE) + 002/003 (outcome card + context assembly, DEFINED-not-built) + 004 (Garmin poller, BACKLOG). The hook + scenario-5 fixture are scaffolded in this epic; full population follows those epics. The golden set must stand **without** memory (AC-3.8).

---

## Definition of Done

- [ ] All buildable-now ACs (S1–S4, S6, S8 replay-only, S7 unit half) covered by at least one passing test; the **existing** `tests/test_eval.py` passes unchanged (additive guarantee, S1)
...
- [ ] Targeted review: verify rule #4 on every eval query path (no privileged bypass; real `scoped_session(viewer_id, granted_ids)`), rule #1 invariants are code-not-judge, the judge is provider-independent, no `conditions/` fixture reaches the graph (rule #3), and `truthfulness.py`'s spine is unchanged in signature
...
- [ ] **Blocked ACs explicitly tracked:** AC-5.6 (judge calibration), AC-7.3 (sensitivity-routing eval-loop), and all of S9 are checked off only after Epics 002/003/004 land — this epic **cannot reach DONE before them**
...
- [ ] Pushed; epic status updated; index row in `docs/epics/README.md` (this is Epic **009**)

---

## Implementation notes

**Epic numbering:** filed as `epic-007-eval-harness-expansion.md` per the orchestrator's filename, but the **canonical epic number is 009** — 006 (Novelty filter), 007 (Readiness filter), and 008 (API tests) are already taken in `docs/epics/README.md`. The H1, README row, and all cross-refs use **009**.
...
- `evals/security.py` — the §7 eval-loop invariants (no-ungranted-node, commons-severance, sensitivity-routing).
...
**Deferred by dependency (specified, not built here):** grant-stop provenance traversal semantics (Stage 8 — note `granted_ids` *parameter* exists today) · party-merge quality metrics (Stage 8) · commons-derived quality references (Stage 9) · rendered-card structured-claim fidelity round-trip (needs a `FeedCard` structured-claim field first).
