...
- [x] `make check` green (`ruff format --check` + ruff + mypy + pytest) — ingestion test count grew (221 → 284), no regression.
- [x] `ingestion/sources/base.py` + `registry.py` exist; `OsmSource`/`NpsSource`/`UsfsSource` refactored behind the contract; `EchoSource` stub present.
- [x] **C5-closure asserted (AC-2.5):** no `"osm"`/`"nps"`/`"usfs"` string literal in `run_pipeline`'s body or the CLI `--source` choices (`test_ac2_5_*`).
- [x] **Modularity proof asserted (AC-6.3):** the `echo` drop-in's diff touches no `run_pipeline` line (`test_ac6_3_*`).
- [x] `match.py` confirmed **unchanged** (AC-4.2, `git diff` empty); `graph/load.py` loader bodies, `ingestion/fetch/*` transports, `ingestion/transform`, hygiene, and `consolidate_osm_segments` body confirmed unchanged. `ingestion/ingest_episode.py` (Track A) byte-untouched.
- [x] Default config (`osm,nps,usfs`, no enrichment) produces **identical** counts to the pre-epic pipeline on the pilot region fixture (AC-5.2) — asserted end-to-end through the real adapters in `test_ac5_2_default_config_counts_through_real_adapters`.
- [x] Targeted adversarial self-review run (6 dimensions × verify pass): **8 raw → 7 confirmed findings**. **1 CRITICAL fixed before commit** (shipped `.env.example` blank `ADVENTURE_USFS_GEOJSON=` was exported as `""` by the Makefile's `. ./.env`, tripping `UsfsSource.from_config`'s fail-loud and aborting the whole default run — now shipped commented-out so a copied `.env` leaves it unset → default path → degrade-to-`[]`). **MODERATE fixed:** added the AC-5.2 end-to-end golden; **MINOR fixed:** adapter `fetch` now wraps the transport so a malformed-200 degrades to `[]` (rule #6 made structural), AC-3.5 import-absence test added, AC-6.1c idempotency test made a real MERGE-dedup check. Verified: Rule #1/#6 (degrade-not-fabricate), Rule #10 (secrets only via `from_config`), SS-10 asymmetry (corpus `from_config` always-returns).
- [x] Atomic commit split: (1) `ingestion/sources/base.py` contract + types; (2) registry + `Settings.from_env` config parsing; (3) three fetchers refactored into adapters + their tests; (4) `run_pipeline` rewired to iterate the registry (spine + conflate + enrichment steps) + CLI derived choices; (5) conformance suite + `echo` stub; (6) epic doc + `docs/epics/README.md` row update.
- [x] `docs/epics/README.md` updated: Epic 012 row added (Phase 1, depends on Stage 3 ingestion); status `DONE ✅`.
