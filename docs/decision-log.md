...
## 47. Phase-C pivot chosen + Supabase Auth adopted (Open Decision #4 closed) ✅ *(2026-07-14 — decided by Josh; brief: `research/auth-provider-decision-brief.md`; epics: 043 · 042 · 044)*
- **The strategic fork is resolved: Phase C ("fill the empty intake") goes before Phase B's remainder.** Rationale (Josh): years of personal activity history exist — importing them is the most direct test of whether personal intelligence actually augments the experience. Phase B is not abandoned: its **ToS/consent + data export/deletion line item is pulled forward as a hard precondition of history import** (a deletion right must precede health-adjacent intake), and the rest of Phase B queues behind the pivot.
- **Supabase Auth adopted per the brief's standing recommendation** — asymmetric JWT verified against JWKS in FastAPI, stable UUID `sub` → `viewer_id` at the edge, **no RLS** (authorization stays in `ScopedSession`/the grant layer), anonymous browsing un-gated by construction. Adopted with the chunk go-ahead; Clerk stays the named fallback if managed-email convenience ever outweighs the local-first ethos.
- **Build order inside the chunk:** ① Epic 043 managed auth → ② Epic 042 manual trip log (BACKLOG→DEFINED) → ③ the pulled-forward ToS/deletion scaffold (needs the 15-min policy chat) → ④ Epic 044 history import (DEFINED, gated on ③) → ⑤ Epic 006 novelty (unblocked by 044's `been_on` producer). Known wrinkle carried openly: hosted personalized ranking currently degrades-and-discloses (the C4 local-judge rule vs. no local model on Render — PR #70); a per-role model-routing decision is owed before the "see the difference" moment works on the hosted app.

## 48. Running history: full import, separate capability books ✅ *(2026-07-14 — decided by Josh; resolves Epic 044's AC-6.1 open sub-decision; epic: 044 S4/S6)*
- **Runs import fully** — Episodes tagged `activity_type:"run"`, `been_on` at full validity (presence is presence), trip memory + novelty pre-warm. Rides/swims/gym stay out of Episodes, counted in the run report.
...
## 50. Map-matching for history import: borrow Leuven, trajectory-similarity MVP first (Open Decision #10 closed) ✅ *(2026-07-14 — decided by Josh after a PO independent-verification pass; plan: `research/map-matching-borrow-plan.md`; epic: 044 S3)*
- **The decision:** bind imported GPX tracks to corpus trails with a **trajectory-to-polyline similarity MVP** (buffer-overlap + directed-Hausdorff/Fréchet, topology-free) that emits a **real confidence gradient**; below threshold → the never-blocks free-floating Episode. **Vendor-borrow LeuvenMapMatching (Apache-2.0)** for the rigorous HMM, **deferred to the topology gate** (B010) and reserved for the hard case only — a single track spanning multiple connected trails. This unblocks Epic 006 early without the topology prerequisite.
- **Why not the full HMM now:** verified against the live schema — no `:Junction`/`CONNECTS_TO` connectivity layer and no `been_on` edge exist yet; Leuven's transition costs need connectivity, so the rigorous path is gated on unbuilt topology. The common case (track ↔ one named trail) is a curve-similarity problem the HMM is overkill for.
- **PO independent verification corrected the spike on three points** (folded into the plan's §0): FMM is Apache-2.0 not MIT; the problem is trajectory-similarity not road map-matching; and buffer matches must **not** all be capped at "low confidence" — a near-complete directed-Hausdorff match is honestly high-confidence, which matters because §48's whole point is producing real `been_on` for novelty. Both libraries are aging (Leuven 2022, FMM 2020) → vendor-port + verify on Python 3.11.
