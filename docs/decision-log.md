...
- **Model tiering:** ✅ Haiku-tier for extraction/normalization, Opus-tier for Curator judgment + the LLM-judge; Sonnet-tier as middle fallback. Exact model IDs in config/`.env` (resolved via Models API), not hardcoded — swappable + A/B-able. Adaptive thinking on the Opus-tier judgment call.
...
- **Cost:** rough estimate **~$0.10–0.18/uncached session**; levers = shortlist cap K · prompt caching (stable prefix + subgraph) · model tiering · condition-TTL caching. ❓ **real budget pending the measurement spike** (run the real flow against the real corpus and measure — do not ship off the estimate). *(the remaining §23 "spike": cost-per-session)*
