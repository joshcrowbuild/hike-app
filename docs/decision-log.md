...
- **Commons write for episodes:** 🔶→**Built (Epic 010, 2026-06-24)** *(on Epic 011's scoped-write seam).* The status glyph stays 🔶 only because `tests/test_commons_doc_lint.py` (Epic 010 AC-1.5) still pins this bullet — flagged for the build lane to retire (see **NEEDS-PM** in PR #1). Forked write on episode creation: person→observation link severed; GPS track endpoint-trimmed (250m strip, 🔶 threshold still tunable); raw pace bucketed into 4 capability bands before commons write. Private episode retains full data. Post-aggregation deletion of individual contribution is not recoverable — disclosed in consent.
...
- **Commons fork:** 🔶→**Built (Epic 010, 2026-06-24).** Glyph pinned to 🔶 by `tests/test_commons_doc_lint.py` (AC-1.5) — flagged to retire (see **NEEDS-PM** in PR #1). `CommonsObservation` written in same transaction as Episode; person link severed at write; 250m endpoint trim; raw pace substituted with 4-band capability label before commons write.
