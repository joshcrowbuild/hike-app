...
> **◆ HOSTED & LIVE — but the live app is a trust liability right now (v10).** Full stack is deployed and the plumbing works: Vercel frontend → Render API (`adventure-planner-api.onrender.com`, `/health` + `/plan` 200, real graph counts) → Aura Neo4j; maps+terrain (016/017) serve real geometry + 3DEP elevation. The Cypher-25/provider/TLS gotchas are fixed (#41/#44). **BUT two P0 blockers mean a real user on their phone sees junk or nothing:** (1) **the corpus re-ingest never ran** — #45's trail-worthiness + conflation fix is merged in code but Aura still holds the pre-fix junk, so `/plan` still suggests non-trails ("Path to School", "Snake Road", "Little Loop Road"); `/health` still reports the pre-fix 1458. (2) **first tap fails** — the Vercel client aborts `/plan` at 10s while Render free-tier cold-starts in 30–60s. Both fixes are cheap and are the release gate. **Altitude:** a north-star vision + path-to-complete + research layer merged (#48); a full audit places us honestly at **Phase A, pre-Milestone-1** ("stop the lies in the substrate"). The architectural distance to the vision is small; the experiential distance is large and inverted — the live deploy currently commits the exact cardinal sin (undated/unsourced data shown as truth) the product exists to refute. Rule: **substrate before surface** — land G1 (safe re-ingest) + G2 (cold-start) before any new surface.
...
**Hosted end-to-end; live app broken at its one job pending two cheap fixes.** Phase-1 backend (001–005, 010–015), design system, app UX, and Maps (016/017) are all on `main` and the deploy plumbing works (Vercel → Render → Aura, real geometry + 3DEP elevation served). The hard moat is genuinely built: invariant-faithful engine, CI-proven query-layer access control, the provenance graph. **Two P0 blockers (the release gate):** **G1** — the corpus re-ingest never ran, so the live corpus is the pre-fix junk (#45 merged in code only); **G2** — first `/plan` tap aborts at the 10s client timeout vs Render's 30–60s cold start. **Phase A (now), pre-Milestone-1** per the new path-to-complete (#48). Phase-A checklist: (1) safe re-ingest + verify [gate], (2) cold-start fix + warm-ping, (3) corroboration wiring (CDP-01 resolved — wire distinct corpus origins into `for_fact`; the machinery already scales it), (4) the 1643→1458 conflation/slug audit, (5) defend-and-secure `/plan` (rate-limit + observability) before any real-user intake. **Vision/path/research layer merged** (#48); CDP-01 spike resolved (#49, docs, pending merge). **Behind Phase A, still design-gated:** Epic 008 (API tests), 006 (novelty — needs `been_on` producer), 007 (readiness), 009 (eval). **Still unmeasured:** R5 cost spike (now measurable against live `/plan`).
