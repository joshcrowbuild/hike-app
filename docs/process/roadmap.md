...
**Last verified:** 2026-07-12 · **Owner:** PO (product-owner lane) · **Repo:** `joshcrowbuild/hike-app` · **Baseline:** tag `v0.1-phase1` @ `c12be36` · `main` is the protected line · **Version:** v16
...
0. **⚠ Warmer cost decision (Render env, 2 min — do first):** set `ADVENTURE_ANON_FEED_CACHE_TTL_S=600` (halves warmer spend + was already queued), then pick: accept the interim spend · raise `ADVENTURE_FEED_WARM_INTERVAL_S` to 480–540 · or set it `0` (warmer off; cold starts return until Epic 040). See `docs/research/r5-cost-model.md` §4.
1. **Branch protection (2 min, laptop browser — GitHub mobile can't):** Settings → Branches → edit the `main` rule → add **five** checks: `eval-replay (source-or-silence)` · `frontend (vitest)` · `a11y (axe, safety stories)` · `bandit` · `secret-scan` (the last one after v14's CLI fix goes green). Until then none of the new gates block a bad merge.
...
3. **R5 cost read (5 min, Render dashboard — now higher-value):** paste a day of `PlanMetrics` log lines (`est_cost_usd`, `warmed`, `feed_cache_hit`) into any Claude session — collapses the cost model's 30× scenario bands (`r5-cost-model.md` §2) into measured numbers and closes R5. While there: note the Render service tier and the `ADVENTURE_PROVIDER_*`/`ADVENTURE_MODEL_*` env values.
4. **Agency API keys (15 min, all free):** provision AirNow + FIRMS + RIDB keys and enable `nps_alerts` in the Render env (Epic 018 S2) — this flips the card's live `not_fetched` rows for air/fire/closures/permits into real, sourced dispositions. The rendering + gate for them already shipped (waves 2–3).
5. **Decisions when ready (no dashboard needed):** ① Supabase auth sign-off (gates Phase C) · ② Open-Meteo commercial license (gates Epic 037 downstream) · ③ 15-min ToS/privacy policy-positions chat (gates the last big Phase-B lane) · ④ readiness (007) design session (Phase D).
6. **Optional/later:** Sentry + uptime accounts (observability's external half).
...
| **R5** | **Latency half MEASURED (Epic 039, 2026-07-08):** /plan was 8.8s cold / 3.7–4.0s on every load; after Wave 1 (anon fast-path · engine-layer anon plan cache · frontend stale-while-revalidate) a returning visitor paints in **0.385s** and a cache hit serves in **0.22–0.73s** (verdict table: `docs/research/feed-first-paint-latency-wave1.md`). Cache-hit calls also spend zero LLM tokens (PlanMetrics `feed_cache_hit`), so the *cost*-per-session half is now partially mitigated AND measurable from Render logs. | **PARTIALLY CLOSED — cost half MODELED (v16), measurement owed** | Bottom-up model with scenario bands in `research/r5-cost-model.md`: organic LLM ~$1–7/mo; **the live B5 warmer is the dominant driver at defaults ($20–470/mo band)**; hosting $7; APIs $0; Aura Free→Pro (+$66/mo) is the structural cliff. One day of PlanMetrics lines pasted from Render logs closes R5 fully. Epic 040 (two-phase, DEFINED) retires the warmer and returns spend to organic. |
